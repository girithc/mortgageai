...
class User:
    ...
    def add_client(self, name, credit_score=0, income_sources=[0.0]*5):
        client = Client(name=name, credit_score=credit_score, income_sources=income_sources)
        self.clients.append(client)

    def update_client(self, name, credit_score=None, index=None, new_income=None):
    ...
    def load_users(filepath=DATABASE_PATH):
        ...
        with open(filepath, 'r', newline='') as f:
            ...
            for row in reader:
                ...
                users_dict[username][1].append(row)
    ...
    def save_user(user, filepath=DATABASE_PATH):
        ...
        for (username, client_name), (user_obj, client_obj) in row_map.items():
            ...
            if client_obj:
                users_out[username].add_client(client_obj.name, 
        ...
        with open(filepath, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['username', 'user_name', 'client_name', 'credit_score',
                             'income1', 'income2', 'income3', 'income4', 'income5', 'total_income'])
